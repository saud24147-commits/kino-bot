...
async def addmovie_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    code = context.args[0]
    video = update.message.video or (update.message.reply_to_message.video if update.message.reply_to_message else None)

    if not video:
        await update.message.reply_text("❌ Video topilmadi. Videoni caption bilan birga yuboring.")
        return
