...
async def send_movie(update_or_query, context: ContextTypes.DEFAULT_TYPE, code: str, chat_id: int):
    ...
    if not entry:
        await context.bot.send_message(chat_id=chat_id, text="Kod noto'g'ri ekan, mendan emas ayb🤷‍♂️")
        return
    ...
    if file_type == "document":
        await context.bot.send_document(chat_id=chat_id, document=file_id, caption="Kayf qilinge😉")
    elif file_type == "video_note":
        await context.bot.send_video_note(chat_id=chat_id, video_note=file_id)
    elif file_type == "photo":
        await context.bot.send_photo(chat_id=chat_id, photo=file_id, caption="Kayf qilinge😉")
    else:
        await context.bot.send_video(chat_id=chat_id, video=file_id, caption="Kayf qilinge😉")
...
async def start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if code:
        ...
        await update.message.reply_text(
            "Ha Mana bo'larkanku🤝\nKinoni kodini yozing endi shefim"
        )
...
async def check_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    not_joined = await get_unsubscribed_channels(user_id, context)
    if not_joined:
        await query.answer("Quyidagi kanallarga a'zo bo'ling🤝\n(Tirikchilikde okam😕)", show_alert=True)
        return
